...
Static Function GetEstRM()
		...
		cQuery+= "		ELSE ''"
		cQuery+= "		END) MFASE,"
		cQuery+= "		CRONOG.CODCOLIGADA,FILIAL.IDINTEGRACAO AS FILIAL_PROTHEUS,CRONOG.IDPRJ,CRONOG.IDTRF,CRONOG.CODTRF,CRONOG.IDPERIODO,CRONOG.NOME,INSUMO.CODISM,CRONOG.DTINICIO,CRONOG.DTFIM,CAST(CRONOG.VALORPLANEJADO AS float) AS VALORPLANEJADO, "+cBancoRM+".dbo.subcontaArvore( CRONOG.CODCOLIGADA, CRONOG.IDPRJ, CRONOG.IDTRF) AS CODCUSTOTAREFA,PROJ.CODCCUSTO AS CODCUSTOPROJETO, COMP.CONTAORC, MINSUMO.CODGIS AS GRUPO,TPRD.CODIGOPRD, SUBSTRING(CONVERT(varchar, CRONOG.DTINICIO ,3),4,5) AS MES_REF FROM "+cBancoRM+".dbo.MXMTAREFACRONOG AS CRONOG " 
...
Static Function GetRecRM()
		...
		cQuery+= "		CRONOG.DTINICIO,CRONOG.DTFIM,CAST(CRONOG.VALORPLANEJADO AS float) AS VALORPLANEJADO, "+cBancoRM+".dbo.subcontaArvore( CRONOG.CODCOLIGADA, CRONOG.IDPRJ, CRONOG.IDTRF) AS CODCUSTOTAREFA,"
